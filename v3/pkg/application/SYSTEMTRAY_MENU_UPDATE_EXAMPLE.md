...
2. **Menu Updates**: When you call `menu.Update()`:
   - The menu structure is rebuilt by clearing and re-adding items
   - The NSMenu object is updated in-place
   - The MenuDelegate notifies macOS that the menu needs refreshing

3. **Real-Time Display**: The NSMenuDelegate's `menuNeedsUpdate:` method is called by macOS:
   - Before the menu is displayed
   - Periodically while the menu is open
   - This ensures the displayed menu shows the latest changes
...
2. **Delegate Implementation**:
   - `menuNeedsUpdate:` - Called when menu needs refreshing
   - `menuWillOpen:` - Called when menu is about to open
